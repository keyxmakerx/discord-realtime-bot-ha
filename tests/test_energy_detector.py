...
def test_job_active_suppresses_energy_idle_timeout() -> None:
    # The false-done fix. This washer's energy meter can read flat for an entire
    # load (it froze/reset on a real capture), so while job_state is reporting an
    # active phase the energy idle-timeout must NOT finish the load — even after
    # the meter has been flat far longer than idle_timeout.
    det = EnergyDetector(idle_timeout=IDLE)
    assert det.observe(0, 14.8, job_is_early=True) == EV_STARTED
    # Meter dead-flat the whole load, but the cloud is reporting phases.
    assert _poll_flat(
        det, 300, 14.8, until=3 * IDLE, job_active=True
    ) is None
    assert det.phase == RUN_ACTIVE
    # The coordinator owns completion here; the explicit 'finish' still works.
    assert det.observe(3 * IDLE + M, 14.8, job_is_finish=True) == EV_FINISHED
    assert det.phase == RUN_IDLE


def test_offline_load_still_finishes_on_idle_timeout_when_job_dark() -> None:
    # With the cloud dark (job_active False), energy remains the only signal, so
    # the idle-timeout backstop must still close an offline load.
    det = EnergyDetector(idle_timeout=IDLE)
